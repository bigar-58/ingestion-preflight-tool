...
def validate_users_csv(path: Path) -> ValidationResult:
    ...
    cols = rel.columns
    missing_cols = [c for c in required_cols not in cols]
    if missing_cols:
    ...
    for col in ["user_id", "email", "signup_date"]:
        cnt = con.execute(f"SELECT COUNT(*) FROM rel WHERE {missing_expr(col)}")
        if cnt > 0:
